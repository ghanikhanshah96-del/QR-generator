...
export function bootGenerator(options = {}) {
  ...
  state.subscribe((_snap, reason) => {
    if (reason === 'replace') {
      editor.syncFromState?.();
      if (typeSelect) {
        typeSelect.value = state.get().type;
        document.querySelectorAll('#qr-type-chips [data-qr-type]').forEach((btn) => {
          btn.setAttribute('aria-pressed', String(btn.getAttribute('data-qr-type') === state.get().type));
        });
      }
      mountFields();
      refreshPayload();
  ...
  return { state, renderer, site: SITE };
}
